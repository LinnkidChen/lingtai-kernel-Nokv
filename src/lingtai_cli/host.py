"""lingtai_cli.host — product assembly / host layer for the agent runtime.

This is the thin host that translates a project workdir (``init.json``) into a
running ``Agent``. It owns the ``lingtai-agent`` CLI behaviour (``run`` /
``check-caps`` / ``log``). ``lingtai.cli`` is a compatibility shim that
re-exports the public names defined here so existing imports and the
``lingtai-agent`` console script keep working unchanged.

Kept deliberately light at import time: the heavy wrapper imports
(``Agent``, ``LLMService``, …) live at module top here only because the legacy
``lingtai.cli`` already imported them eagerly and tests patch them by name. A
bare ``import lingtai_cli`` (the package ``__init__``) does NOT import this
module, so the package itself stays import-light — see
``lingtai_cli.assembly`` for the dependency-light project-state bridge.
"""
...
def main(*, prog: str = "lingtai-agent") -> None:
