...
def _rerender_unread_digest(agent) -> str | None:
    ...
    publish_notification(
        ...
        instructions=(
            ...
            "If you are about to start a long primary tool call (e.g. "
            "a slow bash, daemon, web_search, or large file write) and a "
            "preview is truncated, attach a read to that primary call via the "
            "reserved 'secondary' field so you have the full body before the "
            "primary starts: use secondary={tool:'email', args:{action:'read', "
            "email_id:[...]}}. The bounded read result is forwarded under "
            "_secondary.result on the primary tool result. The secondary "
            "channel is read-only — to acknowledge or answer the sender, call "
            "email reply directly as a normal tool. "
