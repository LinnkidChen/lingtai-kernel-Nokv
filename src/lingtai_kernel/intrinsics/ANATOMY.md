...
- [`intrinsics/system/`](system/ANATOMY.md) — runtime, lifecycle, synchronization. Nap, refresh (preset hot-reload + authorization gate), karma-gated lifecycle actions on other agents (sleep, lull, suspend, cpr, interrupt, clear, nirvana), preset listing, voluntary notification reads (`action="notification"` returns a placeholder dict — the canonical `notifications` + `_notification_guidance` payload is stamped onto the same result by the meta-block post-hook), and a deprecated `dismiss` no-op. **Conceptual home of the notification surface** — re-exports `publish_notification` / `clear_notification` from the kernel-root `notifications.py` so any in-process producer submits through one canonical entry point. Decomposed in `e206dbc` from a 641-line `system.py` into `nap.py`, `preset.py`, `karma.py`, `notification.py`, `schema.py`, `__init__.py` with explicit dispatch table.

- [`intrinsics/notification/`](notification/ANATOMY.md) — the **standalone notification surface**, split out of `system` so the notification verbs have their own mandatory tool. A thin façade: `check` returns the same placeholder dict as `system(action="notification")` (the meta-block stamps the live payload onto it); `dismiss` delegates to `system.notification._dismiss` with `_invoked_by="notification"`; `summarize` delegates to the single `system.summarize._summarize`. No notification logic is reimplemented — `system(action="notification"|"dismiss"|"summarize")` remain compatibility aliases routing into the same functions, so the two tools produce identical results (only the provenance log line differs). All #424 large-result-reminder guards live in `notifications.dismiss_channel` / `clear_large_result_reminders` and therefore hold through both tools by construction. Files: `__init__.py` (façade + dispatch), `schema.py` (reuses `system_tool.*` i18n strings for shared params, adds `notification_tool.*`).
