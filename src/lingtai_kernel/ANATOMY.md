...
- `tool_executor.py` — `ToolExecutor`. Synchronous/parallel tool dispatch, reasoning/commentary stripping, ToolCallGuard policy/preflight evaluation, LoopGuard duplicate advisory metadata, timing, enriched model-visible error payloads, result spilling, provider-visible result construction, ACTIVE-turn tool-call progress metadata, and event-sourced tool-call lifecycle tracing. Stamps a permanent **`_tool`** identity block into every dict tool result (`id`, `timestamp`, `elapsed_ms`, `char_count`, optional `spilled_char_count`, and optional abnormal `status`); `char_count` is a **result-intrinsic** count — the kernel auxiliary keys (`_tool`, `_runtime`, `_runtime_pending`, `_tool_result_metadata`, `_advisory`, `notifications`/`_notification_guidance`, the batch progress notice, and any top-level `active_turn_tool_calls`) are excluded so the figure reflects the model-visible tool payload; `spilled_char_count` appears only when the result was saved to a sidecar and records the original spilled payload size. The latest-only **`_runtime`** block is NOT stamped here: `stamp_meta()` records a transient `_runtime_pending` snapshot per result, which `meta_block.attach_active_runtime()` promotes into a real `_runtime` block (`state` = kernel runtime state + `elapsed_ms` + `active_turn_tool_calls`, plus `guidance` from `guidance.json`) onto the latest dict result at the tool-batch boundary in `base_agent/turn.py`, stripping the prior holder's `_runtime`. The ACTIVE-turn counter therefore lives under `_runtime.state.active_turn_tool_calls` (latest-only) rather than being repeated top-level; only the transient `active_turn_tool_call_notice` is attached to the triggering result. `summarize_instruction` and `tool_name` are included in `_tool_result_metadata` **only** when `long_tool_result=True`; ordinary results carry a minimal `_tool_result_metadata` (`tool_call_id`, `char_count`, `threshold_chars`, `long_tool_result`, `hint`) and rely on `_tool` for identity, while generic summarization discipline lives in `_runtime.guidance` instead. Each provider/top-level call gets a `tool_trace_id` and logs proposal/normalization/validation/guard approval-or-denial/dispatch/result visibility boundaries. With the default empty guard chain, behavior remains pass-through (`policy="default_allow"`); when a guard denies, `ToolExecutor` synthesizes a structured `ToolCallGuardDenied` tool result instead of dispatching the handler, preserving provider tool-call/tool-result pairing for future policy rejections. Error results returned to the model include repair context (`error_type`, `error_phase`, `tool_trace_id`, normalized `tool_args`/`arg_keys`, bounded traceback tail for executor exceptions) plus a nested `tool_error` recovery block with summary/reason/retryability/guidance, while durable lifecycle logs remain the forensic record. It executes only provider/top-level tool calls; the old reserved nested `secondary` side channel has been removed. For compatibility with stale model output or old cached schemas, `_execute_single` and `_execute_parallel` drop a top-level `secondary` argument before dispatch and log `deprecated_secondary_ignored` instead of executing any nested communication call.
...
- `meta_block.py` — meta-block rendering (the structured prefix the kernel injects into LLM messages with state, time, stamina, etc.). Also owns `build_runtime_guidance()` (loads, schema-validates via `validate_runtime_guidance()`, and caches `guidance.json` from the package resource; degrades to `{}` on a missing/unreadable/malformed resource), `stamp_meta()` (records the per-tool runtime snapshot under the transient `result["_runtime_pending"]`), `attach_active_runtime()` (the latest-only moving holder: promotes the newest dict result's `_runtime_pending` into a real `_runtime` block — `state` + `guidance` — sourcing `active_turn_tool_calls` from the executor guard, and strips `_runtime` from the prior holder; mirrors `attach_active_notifications`), and `now_iso_plain()` (UTC ISO timestamp without agent context). Notification payloads are deliberately kept **separate** from `_runtime` — they are channel-owned, latest, on-demand (see `attach_active_notifications`).
- `guidance.json` — **package resource** (declared in `pyproject.toml` under `lingtai_kernel`). Contains `schema_version`, `guidance_version`, `priority`, `render_mode`, and an ordered list of `sections` (`id`, `title`, `body`). Loaded once per process by `build_runtime_guidance()` into a module-level cache. Current sections: `summarize_discipline`, `tool_loop_guard`, `context_discipline`, `error_recovery`, `notification_discipline`. Surfaced to the model inside `result["_runtime"]["guidance"]` on the latest tool result each ACTIVE turn.
