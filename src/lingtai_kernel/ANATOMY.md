...
- `nudge/` — **per-agent periodic checks** that publish to the shared `.notification/nudge.json` channel when something needs the agent's attention. `__init__.py:run_checks(agent)` is called once per heartbeat tick from `base_agent/lifecycle.py:_heartbeat_loop` (~line 320, wrapped in try/except). Each check is a self-contained module under `nudge/` exposing `check(agent) -> None`; throttling and dedupe state live on the agent as `agent._nudge_<name>_state`. Checks call `nudge.upsert(agent, kind, body)` / `nudge.remove(agent, kind)` to mutate the multi-entry `nudge.json` payload under a lazy per-agent RMW lock. Today's only check is `kernel_version.py`: compares frozen `lingtai.__version__` against `importlib.metadata.version("lingtai")` to detect a newer wheel on disk and nudges the agent to refresh. Agent dismisses everything at once with `system(action='dismiss', channel='nudge')`. See `nudge/ANATOMY.md` for the "add a new nudge" recipe.
...
- `tool_executor.py` — `ToolExecutor`. Synchronous/parallel tool dispatch, reasoning stripping, timing, error capture, and the privileged reserved **read-only** `secondary` communication side channel: `_validate_secondary` enforces the communication whitelist, the `{read}`-only action policy (`send`/`reply` are rejected with a "secondary action is not allowed" error), and the recursive-key ban; `_execute_secondary` rejects excluded primary tools at runtime, then dispatches valid read calls mechanically before the primary, bypassing LoopGuard for the secondary itself while never blocking the primary; `_execute_single` pops `secondary` before the primary LoopGuard key and reports the outcome under the top-level `_secondary` field — for `action="read"` it forwards a bounded slice of the read payload under `_secondary.result` (capped by `SECONDARY_READ_RESULT_MAX_BYTES`). Because send/reply never dispatch, comm-handler internals (e.g. message_ids) can never leak. `_execute_parallel` runs all allowed secondaries in a sequential pre-pass before worker threads.
- `secondary_tools.py` — policy and schema helpers for reserved nested `secondary` calls. The channel is **read-only**: communication-tool whitelist (`email`/`telegram`/`wechat`/`feishu`/`whatsapp`), action whitelist `{read}` per tool (no `send`/`reply` — human-facing writes stay on the agent's ordinary top-level communication tools), primary-tool eligibility exclusions, read-result cap constant, and the injected JSON-schema property (`secondary_tools.py:18-103`).
...
- `prompt.py` — `SystemPromptManager` plus `build_system_prompt` / `build_system_prompt_batches`. Composes the system prompt from identity, capabilities, intrinsics, pad, rules. Default render order (`prompt.py:52-68`): principle → covenant → tools → **substrate** → procedures → comment, then rules → brief → skills → knowledge → identity → **character** → pad. `substrate` sits **right after** `tools` so it provides a compact resident operating model and routes expanded runtime/procedure guidance to the `system-manual` skill. `character` (the agent's self-authored identity from `system/lingtai.md`, written by `_lingtai_load`) renders right after the mechanical `identity` section and before `pad` — distinct from both `covenant` (operator contract, Batch 1) and `identity` (name/nickname/manifest). The kernel ships `lingtai/prompts/substrate.md` as the packaged compact default (issue #39) and `src/lingtai/intrinsic_skills/system-manual/SKILL.md` as the expanded manual; the `Agent` subclass auto-seeds `system/substrate.md` from it on first boot — no init.json opt-in required.
...
The canonical live payload is built by `meta_block.build_notification_payload` (`meta_block.py:180-222`) and used by both delivery surfaces. The synthesized-pair envelope comes from `BaseAgent._inject_notification_pair` (`base_agent/__init__.py:1019-1184`): it adds `_synthesized: True` (also written as the `synthesized=True` flag on the `ToolResultBlock`) around the canonical payload so the agent can distinguish kernel-injected reads from voluntary `system(action="notification")` calls when reading conversation history. The kernel adds a top-level `_notification_guidance` field plus a source-specific `_notification_guidance` field into each per-source block under `notifications` — this is kernel safety framing, separate from the producer's own optional `instructions` field (see Producer contract below). ACTIVE tool-result delivery calls `meta_block.attach_active_notifications` (`meta_block.py:336-454`, wired from `base_agent/turn.py:926-937`), which attaches that same canonical payload to the latest dict-shaped tool result; it does not build a compact/preview-only representation.

### Communication responsiveness and secondary tool calls

Notification previews deliberately trade completeness for latency: they let the agent notice that something arrived, not necessarily act safely from the preview alone. The operational rule lives in the substrate: answer directly only when the newest human message is complete and unambiguous; otherwise read the producer. The reserved `secondary` channel is the runtime bridge between those rules and long primary work, and it is **read-only** — it fetches the full body of a just-notified message before the primary starts, but cannot contact a human. Acknowledgements and replies stay on the agent's ordinary top-level communication tools. Eligible primary tool schemas receive a narrow nested communication-call property from `secondary_tools.py` (`secondary_tools.py:23-29`, `secondary_tools.py:65-103`): only `email`/`telegram`/`wechat`/`feishu`/`whatsapp`, only the `read` action, no nested `secondary`. `ToolExecutor._validate_secondary` enforces the whitelist, the `{read}`-only action policy, and the recursive-key ban (`tool_executor.py:185-249`), `_execute_secondary` runs the read before the primary while catching all failures (`tool_executor.py:251-370`), and `_execute_single` / `_execute_parallel` attach the outcome to the primary result under `_secondary` (`tool_executor.py:416-`, `tool_executor.py:559-`). For `action="read"`, the executor forwards a bounded slice under `_secondary.result` (`tool_executor.py:325-335`); `send`/`reply` are rejected before dispatch, so handler internals such as message IDs cannot leak. This is intentionally not a general nested-workflow engine: it exists to fetch full message content without abandoning the primary task.
