...
- `tool_executor.py` — `ToolExecutor`. Synchronous/parallel tool dispatch, reasoning/commentary stripping, ToolCallGuard policy/preflight evaluation, LoopGuard duplicate advisory metadata, timing, enriched model-visible error payloads, result spilling, provider-visible result construction, ACTIVE-turn tool-call progress metadata, and event-sourced tool-call lifecycle tracing. Stamps a permanent **`_meta.tool_meta`** identity block into every dict tool result (`id`, `timestamp`, `elapsed_ms`, `char_count`, optional `spilled_char_count`, and optional abnormal `status`); `char_count` is a **result-intrinsic** count — the whole kernel `_meta` envelope and the transient top-level scaffolding (`_runtime_pending`, `_advisory`, the batch progress notice, and any top-level `active_turn_tool_calls`) are excluded so the figure reflects the model-visible tool payload; `spilled_char_count` appears only when the result was saved to a sidecar and records the original spilled payload size. The latest-only **`_meta.agent_meta`** / **`_meta.guidance`** blocks are NOT stamped here: `stamp_meta()` records a transient `_runtime_pending` snapshot per result, which `meta_block.attach_active_runtime()` promotes into `_meta.agent_meta` (kernel runtime state + `elapsed_ms` + `active_turn_tool_calls`) and `_meta.guidance` (from `guidance.json`, with `meta_readme` appended as an ordered `guidance.sections` chapter) onto the latest dict result at the tool-batch boundary in `base_agent/turn.py`, stripping the prior holder's `agent_meta`/`guidance`. The ACTIVE-turn counter therefore lives under `_meta.agent_meta.active_turn_tool_calls` (latest-only) rather than being repeated top-level; only the transient `active_turn_tool_call_notice` is attached to the triggering result. Tool result metadata is represented by formal blocks nested under a single **`_meta`** envelope: permanent per-result `_meta.tool_meta`, latest-only `_meta.agent_meta`, latest-only `_meta.guidance` (ordered prompt-like sections, including the `meta_readme` section), and channel-owned latest-only `_meta.notifications`/`_meta.notification_guidance`; the legacy `_tool_result_metadata` compatibility block is removed. Long-result reminders are notification events, not provider-visible metadata. Each provider/top-level call gets a `tool_trace_id` and logs proposal/normalization/validation/guard approval-or-denial/dispatch/result visibility boundaries. With the default empty guard chain, behavior remains pass-through (`policy="default_allow"`); when a guard denies, `ToolExecutor` synthesizes a structured `ToolCallGuardDenied` tool result instead of dispatching the handler, preserving provider tool-call/tool-result pairing for future policy rejections. Error results returned to the model include repair context (`error_type`, `error_phase`, `tool_trace_id`, normalized `tool_args`/`arg_keys`, bounded traceback tail for executor exceptions) plus a nested `tool_error` recovery block with summary/reason/retryability/guidance, while durable lifecycle logs remain the forensic record. It executes only provider/top-level tool calls; the old reserved nested `secondary` side channel has been removed. For compatibility with stale model output or old cached schemas, `_execute_single` and `_execute_parallel` drop a top-level `secondary` argument before dispatch and log `deprecated_secondary_ignored` instead of executing any nested communication call.
...
- `meta_block.py` — meta-block rendering (the structured prefix the kernel injects into LLM messages with state, time, stamina, etc.). Owns the **`_meta` envelope** constants/helpers (`_meta_block()`, the `tool_meta`/`agent_meta`/`guidance`/`notifications`/`notification_guidance` key names) and `build_meta_readme()` (the latest-only self-description rendered as a `meta_readme` section inside `_meta.guidance.sections`). Also owns `build_runtime_guidance()` (loads, schema-validates via `validate_runtime_guidance()`, and caches `guidance.json` from the package resource; degrades to `{}` on a missing/unreadable/malformed resource), `stamp_meta()` (records the per-tool runtime snapshot under the transient `result["_runtime_pending"]`), `attach_active_runtime()` (the latest-only moving holder: promotes the newest dict result's `_runtime_pending` into `_meta.agent_meta` and `_meta.guidance` — guidance always appends `meta_readme` to `sections` — sourcing `active_turn_tool_calls` from the executor guard, and strips those blocks from the prior holder; mirrors `attach_active_notifications`), and `now_iso_plain()` (UTC ISO timestamp without agent context). Notification payloads share the same `_meta` envelope but are kept **separate** from `agent_meta`/`guidance` — they are channel-owned, latest, on-demand (see `attach_active_notifications`).
- `lingtai/prompts/guidance.json` — **prompt package resource** (declared in `pyproject.toml` under `lingtai`, alongside `procedures.md` and `substrate.md`). Contains `schema_version`, `guidance_version`, `priority`, `render_mode`, and an ordered list of `sections` (`id`, `title`, `body`). Loaded once per process by `build_runtime_guidance()` into a module-level cache. Current section: `summarize_best_practice`, carrying high-attention tool-result summarization guidance that used to live in resident `procedures.md`. Surfaced to the model inside `result["_meta"]["guidance"]` on the latest tool result each ACTIVE turn, with the kernel-injected `meta_readme` appended as an ordered section. The `Agent` wrapper mirrors the same packaged resource into each workdir as `system/guidance.json` on boot/refresh so TUI surfaces can read the active guidance without importing kernel internals.
...
- Each actual-block injection also records a durable `notification_block_injected` event with `mode`, `call_id`, `sources`, and the full persisted `_meta` envelope (`tool_meta`, `agent_meta`, `guidance`, `notifications`, `notification_guidance`) so `/notification` can replay the same metadata shape the model saw live (`base_agent/__init__.py:1476`, `base_agent/turn.py:1443`).
...
The optional `instructions` field is the producer-side directive — text describing what the agent must do to dismiss or act on the notification. It rides with the payload so each producer owns its own dismissal contract; generic frontend / kernel code does not need to know per-producer rules. Email uses it for "call read or dismiss to clear"; soul flow sets it to advise that voices are inner monologue (no dismissal needed); MCP servers can carry their own. Separately from `instructions`, the kernel injects its own `notification_guidance` text both at the `_meta` level and into each per-source block under `_meta.notifications` when it synthesizes the wire pair — that is kernel-side safety framing about source provenance and verification, not a producer dismissal contract.
