...
- `tool_executor.py` — `ToolExecutor`. Synchronous/parallel tool dispatch, reasoning/commentary stripping, LoopGuard duplicate blocking, timing, error capture, result spilling, provider-visible result construction, and event-sourced tool-call lifecycle tracing. Each provider/top-level call gets a `tool_trace_id` and logs proposal/normalization/validation/pass-through approval/dispatch/result visibility boundaries without adding approval gates or changing execution semantics. It executes only provider/top-level tool calls; the old reserved nested `secondary` side channel has been removed. For compatibility with stale model output or old cached schemas, `_execute_single` and `_execute_parallel` drop a top-level `secondary` argument before dispatch and log `deprecated_secondary_ignored` instead of executing any nested communication call.
