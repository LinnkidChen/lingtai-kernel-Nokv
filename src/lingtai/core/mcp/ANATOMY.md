...
- `mcp/__init__.py` — MCP registry management and tool surface. `get_description`, `get_schema`, `make_handler` (the single-source-of-truth `mcp`-tool handler factory — `setup()` registers `make_handler(agent)`, and the SDK tool-config bundle bridge `lingtai.core.mcp_bundle` hosts the *same* closure — stage 3F), `setup`. Key functions: `validate_record` (`mcp/__init__.py:82-125`), `validate_registry_line` (`mcp/__init__.py:128-138`), `read_registry` (`mcp/__init__.py:149-186`), `decompress_addons` (`mcp/__init__.py:201-257`), `_build_registry_xml` (`mcp/__init__.py:274-299`), `_reconcile` (`mcp/__init__.py:306-342`).
...
mcp/__init__.py
  ...
  └── Tool surface
      ...
      ├── make_handler()            — single-source-of-truth mcp-tool handler factory (stage 3F)
      └── setup()                   — registers make_handler(agent), runs initial _reconcile
