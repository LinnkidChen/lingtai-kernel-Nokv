...
| `guard_wiring.py` | Advisory-first wrapper wiring of the SDK guard bridge (stage 18, C3) — installs an advisory `ToolCallGuard` built from declared bundle manifests onto the Stage-16 `BaseAgent._tool_call_guard` seam. Stage 19 adds provenance tracking so a re-wire with no manifests resets only a wrapper-installed guard (never a host/manual one). Stage 20 populates `core_manifest_registry` (system/psyche/soul providers from `lingtai_sdk.core_bundles`) plus `collect_core_bundle_manifests`; default live wiring now includes these intrinsic core manifests as warnings (`include_core=True`) while unknown/non-core tools remain fail-open. |
...
**`guard_wiring.py`** — `DEFAULT_LIVE_GUARD_MODE` (advisory) · `default_manifest_registry` (empty capability registry) · `core_manifest_registry` (Stage 20; `{system,psyche,soul}` → `lingtai_sdk.core_bundles` providers) · `collect_core_bundle_manifests` (Stage 20; returns the core manifests directly, ungated by `_capabilities` since core surfaces are intrinsics, fail-open per provider) · `collect_agent_bundle_manifests` (walk `_capabilities`, fail-open per provider) · `install_bundle_guard` (build advisory `ToolCallGuard` via `lingtai_sdk.guard_bridge.tool_call_guard_from_manifests`, write to `_tool_call_guard`, tag provenance `PROVENANCE_FLAG`/`PROVENANCE_SOURCE` when manifests present) · `reset_bundle_guard` (Stage 19; restore a default pass-through `ToolCallGuard` and clear provenance) · `wire_agent_guard` (live entry point; Stage20 `include_core=True` by default so core tools warn but never block; `include_core=False` recovers pure pass-through; skips host/manual guards and fail-opens on errors). Invoked by `Agent._wire_bundle_guard` near the end of `__init__` and `_setup_from_init`.
...
**Outbound — SDK (wrapper → SDK edge, allowed):** `guard_wiring.py` → `lingtai_sdk.guard_bridge.{GuardPolicyMode, tool_call_guard_from_manifests}`, `lingtai_sdk.capabilities.BundleManifest`, and (Stage 20) `lingtai_sdk.core_bundles.{system_bundle, psyche_bundle, soul_bundle, core_bundle_names}`. The wrapper may depend on the SDK; the kernel must never import the SDK, so the guard chain is built wrapper-side and installed onto the kernel's `_tool_call_guard` seam — no `lingtai_kernel → lingtai_sdk` inversion. `guard_wiring.py` also imports `lingtai_kernel.tool_call_guard.ToolCallGuard` (wrapper → kernel, allowed) to construct the Stage-19 default pass-through reset guard.
