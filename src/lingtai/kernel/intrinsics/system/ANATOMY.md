...
- `schema.py` — Tool registration.
  ...
  - `get_schema()` — returns JSON schema for the system tool. Action enum includes `dismiss` plus `channel`/`force` parameters for generic notification clearing and `event_id`/`ref_id` for atomic single-event removal from `system.json`; legacy `ids` remains handler-only and is no longer taught in schema.
