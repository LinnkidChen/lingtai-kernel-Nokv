...
This repo contains both layers, published as a single `lingtai` PyPI package. The
kernel now lives **in-tree** at `lingtai.kernel` (`src/lingtai/kernel/`),
relocated from the former top-level `lingtai_kernel` package as a hard cut — there
is no compatibility shim and `lingtai_kernel` is no longer an importable alias.
The canonical kernel import root is `lingtai.kernel`.

- **`lingtai.kernel`** (`src/lingtai/kernel/`) — minimal agent runtime. Contains BaseAgent, intrinsics, LLM protocol (ABCs + service), mail/logging services, and core utilities. Zero hard dependencies — usable on its own via `import lingtai.kernel` (the bare `lingtai` namespace `__init__` it loads is itself import-light).
...
The kernel must never import from the `lingtai` wrapper layer (`lingtai.agent`, `lingtai.capabilities`, `lingtai.core`, `lingtai.llm` adapters, `lingtai.services`) — the dependency is strictly one-directional.
...
- **The kernel anatomy tree** is rooted at [`src/lingtai/kernel/ANATOMY.md`](src/lingtai/kernel/ANATOMY.md) — that file is itself just-an-anatomy of `src/lingtai/kernel/`, following the same 6-section template as every other anatomy. Its Composition section enumerates the direct children. Descend from there.
...
- **`lingtai.kernel.config`** — `AgentConfig` dataclass. Key fields: `max_turns`, `provider`, `model`, `api_key`, `base_url`, `retry_timeout`, `aed_timeout` (max seconds in STUCK before ASLEEP, default 360), `max_aed_attempts` (default 3), `thinking_budget`, `data_dir`, `soul_delay`, `language`, `stamina` (max uptime before ASLEEP), `context_limit`, `molt_pressure`, `molt_prompt`. Host app injects resolved values; no file-based config inside lingtai.
- **`lingtai.kernel.prompt`** — Builds system prompt from base template + `SystemPromptManager` sections + MCP tool descriptions.
- **`lingtai.kernel.loop_guard`** — `LoopGuard` class. Duplicate tool call tracking, total call limit enforcement, invalid tool detection, escalating warnings (dup_free_passes → warnings → hard block).
- **`lingtai.kernel.handshake`** — `is_agent(path)`, `is_alive(path)`, `is_human(path)`, `manifest(path)` — agent discovery and health checks via filesystem.
